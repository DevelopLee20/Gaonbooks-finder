...
def search(event=None):
    ...
    if user_input.startswith("₩") or user_input.startswith("`"):
        try:
            index_number = (
                int(user_input[2:].strip()) - 1
            )  # 1부터 시작하는 인덱스를 0부터 시작하도록 조정
            if last_filtered_df is not None and 0 <= index_number < len(
                last_filtered_df
            ):
    ...
    filtered_df = df[
        df["도서명(저자)"].str.contains(user_input, case=False, na=False)
    ].copy()
    ...
    if not filtered_df.empty:
        ...
        filtered_df["D-일수"] = (today - filtered_df["주문"]).dt.days.apply(
            lambda x: f"D-{int(x)}" if pd.notna(x) else ""
        )
        filtered_df = filtered_df[
            ["연번", "도서명(저자)", "출판사", "위치", "주문_MM-DD", "D-일수"]
        ]
